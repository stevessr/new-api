...
func TestApplyResponsesWSUpstreamHeaders(t *testing.T) {
	...
	t.Run("OpenAI Responses header is not forced", func(t *testing.T) {
		header := http.Header{}
		applyResponsesWSUpstreamHeaders(header, appconstant.ChannelTypeOpenAIResponse)
		assert.Empty(t, header.Get("OpenAI-Beta"))
	})
}

func TestResponsesWSChannelTypes(t *testing.T) {
	assert.True(t, isResponsesWSChannelType(appconstant.ChannelTypeOpenAI))
	assert.True(t, isResponsesWSChannelType(appconstant.ChannelTypeOpenAIResponse))
	assert.True(t, isResponsesWSChannelType(appconstant.ChannelTypeCodex))
	assert.False(t, isResponsesWSChannelType(appconstant.ChannelTypeGemini))
	assert.IsType(t, &openai.Adaptor{}, GetAdaptor(appconstant.APITypeOpenAIResponse))
}
...
func TestDialResponsesWebSocketUpstreamSupportsOpenAIResponseChannel(t *testing.T) {
	t.Setenv("NO_PROXY", "*")
	t.Setenv("no_proxy", "*")
	receivedHeaders := make(chan http.Header, 1)
	upgrader := websocket.Upgrader{}
	...
	info := &relaycommon.RelayInfo{
		...
		IsStream:       true,
		ChannelMeta: &relaycommon.ChannelMeta{
			ChannelType:    appconstant.ChannelTypeOpenAIResponse,
			ChannelBaseUrl: server.URL,
			ApiKey:         "responses-key",
			Organization:   "org-responses",
		},
	}

	target, apiErr := dialResponsesWebSocketUpstream(c, &openai.Adaptor{}, info)
	require.Nil(t, apiErr)
	require.NotNil(t, target)
	defer target.Close()
	header := <-receivedHeaders
	assert.Equal(t, "Bearer responses-key", header.Get("Authorization"))
	assert.Equal(t, "org-responses", header.Get("OpenAI-Organization"))
	assert.Empty(t, header.Get("OpenAI-Beta"))
}
